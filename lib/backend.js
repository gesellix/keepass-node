        ...
        if (_.isEmpty(password)) {
          ...
          keepass.getDatabaseRaw(databaseName, password)
              .then(function (ignored) {
                      ...
                      var token = jwt.sign({payload: encryptedPayload}, secret, {expiresInMinutes: 5});
                      res.json({jwt: token});
                    }, function (reason) {
    ...
    app.put('/:filename/:parentGroup/group/:group',
            function (req, res) {
              ...
              if (!keepass.exists(databaseName)) {
                ...
                if (!parentGroupId) {
                  res.status(409).send({msg: "needs a parent group UUID"});
                }
                else if (!group.UUID || group.UUID != groupId) {
